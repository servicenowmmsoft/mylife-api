...
import REVIEW from '../models/reReviews.model.js';
...
export function GETALL(req, res) {
    ...
    if (errorAuth != "") {
        ...
        REVIEW.find().sort({ created: -1 }).then((results) => {
...
export function SEARCH(req, res) {
    ...
    if (errorAuth != "") {
        ...
        USER.find().then((resUser) => {
            var users = resUser;
            REVIEW.find(queries).count().then((total) => {
                REVIEW.find(queries, fields).skip(startIndex).limit(limit).sort({ created: sortBy }).then((results) => {
                    ...
                    return res.status(200).json({
                        success: true,
                        message: 'Search REVIEW by STARTINEX , LIMIT and FIELDS',
                        results: results,
                        total: total,
...
export function UPDATE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        REVIEW.find({ _id: id }).then(results => {
            if (results && results.length <= 0) {
                ...
                REVIEW.find({ title: title }).then(result => {
                    ...
                    if (!isDuplicate) {
                        ...
                        REVIEW.updateOne({ _id: id }, { $set: updateRecord }).exec().then(() => {
                            res.status(200).json({
                                success: true,
...
export function DELETE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        const id = req.params.id;
        REVIEW.findByIdAndRemove(id).exec().then((result) => {
            if (result) {
