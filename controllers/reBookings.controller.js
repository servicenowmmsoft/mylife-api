...
import BOOKING from '../models/reBookings.model.js';
...
export function GETALL(req, res) {
    ...
    if (errorAuth != "") {
        ...
        BOOKING.find().sort({ created: -1 }).then((results) => {
            return res.status(200).json({
                success: true,
                message: 'A list of all BOOKING',
                results: results,
            });
        }).catch((err) => {
            res.status(500).json({
...
export function SEARCH(req, res) {
    ...
    if (errorAuth != "") {
        ...
        USER.find().then((resUser) => {
            var users = resUser;
            BOOKING.find(queries).count().then((total) => {
                BOOKING.find(queries, fields).skip(startIndex).limit(limit).sort({ created: sortBy }).then((results) => {
                    ...
                    return res.status(200).json({
                        success: true,
                        message: 'Search BOOKING by STARTINEX , LIMIT and FIELDS',
                        results: results,
                        total: total,
...
export function UPDATE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        BOOKING.find({ _id: id }).then(results => {
            if (results && results.length <= 0) {
                ...
                BOOKING.find({ title: title }).then(result => {
                    ...
                    if (!isDuplicate) {
                        ...
                        BOOKING.updateOne({ _id: id }, { $set: updateRecord }).exec().then(() => {
                            res.status(200).json({
                                success: true,
...
export function DELETE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        const id = req.params.id;
        BOOKING.findByIdAndRemove(id).exec().then((result) => {
            if (result) {
