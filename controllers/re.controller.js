...
import Realestate from '../models/re.model.js';
...
function convertSlugByTitle(title) {
    return title.toLowerCase().replace(/\s/g, '-').replace("%", "percent").replace("+", "plus");
}
...
export function CREATE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        Realestate.find({ title: title }).then(result => {
            if (result && result.length > 0) {
                return res.status(400).json({
                    success: false,
                    message: `Can not create with duplicate title = ${title}`,
                });
            }
            else {
                const record = new Realestate({
                    _id: mongoose.Types.ObjectId(),
                    slug: convertSlugByTitle(title),
                    created: Date.now(),
...
export function GETALL(req, res) {
    ...
    if (errorAuth != "") {
        ...
        Realestate.find().sort({ created: -1 }).then((results) => {
            return res.status(200).json({
                success: true,
                message: 'A list of all Realestate',
                results: results,
            });
        }).catch((err) => {
            res.status(500).json({
...
export function SEARCH(req, res) {
    ...
    if (errorAuth != "") {
        ...
        USER.find().then((resUser) => {
            var users = resUser;
            Realestate.find(queries).count().then((total) => {
                Realestate.find(queries, fields).skip(startIndex).limit(limit).sort({ created: sortBy }).then((results) => {
                    ...
                    return res.status(200).json({
                        success: true,
                        message: 'Search Realestate by STARTINEX , LIMIT and FIELDS',
                        results: results,
                        total: total,
...
export function UPDATE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        Realestate.find({ _id: id }).then(results => {
                ...
                Realestate.find({ title: title }).then(result => {
                    ...
                    if (!isDuplicate) {
                        ...
                        if (updateRecord.title) {
                            updateRecord.slug = convertSlugByTitle(updateRecord.title);
                        }
                        Realestate.updateOne({ _id: id }, { $set: updateRecord }).exec().then(() => {
                            res.status(200).json({
                                success: true,
                                message: 'Realestate is updated',
                                result: updateRecord,
                            });
                        }).catch((err) => {
                            res.status(500).json({
...
export function DELETE(req, res) {
    ...
    if (errorAuth != "") {
        ...
        const id = req.params.id;
        Realestate.findByIdAndRemove(id).exec().then((result) => {
            if (result) {
